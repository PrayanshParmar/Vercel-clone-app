...
const ThemedImage = ({ width, height, folder }: ThemedImageProps) => {
    ...
    React.useEffect(() => {
        if (theme === 'system') {
            ...
            setSrc(systemTheme === 'dark' ? `/${folder}/light/light.png` : `/${folder}/dark/dark.png`);
        } else {
            // For explicitly set themes (light or dark)
            setSrc(theme === 'dark' ? `/${folder}/light/light.png` : `/${folder}/dark/dark.png`);
        }
